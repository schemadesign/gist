...
// get a regional map's graphData
router.get('/graph-data', addDataPrepOptions, function (req, res, next) {
    var controller = require('../../../../controllers/client/data_preparation/regional_map');

    controller.BindData(req.dataSource, req.dataPrepOptions, function (err, data) {
        if (err) {
            winston.error('Error getting bind data for json-api datasource regional map graph-data , err: %s', err);
            return next(err);
        }
