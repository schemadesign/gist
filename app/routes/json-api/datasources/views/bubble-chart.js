...
// get a bubble chart's graphData
router.get('/graph-data', addDataPrepOptions, function (req, res, next) {
    var controller = require('../../../../controllers/client/data_preparation/cartesian_charts/bubble_chart');

    controller.BindData(req.dataSource, req.dataPrepOptions, function (err, data) {
        if (err) {
            winston.error('Error getting bind data for json-api datasource bubble chart graph-data , err: %s', err);
            return next(err);
        }
